function [Infstruct,fvec,Scarr] = sload(varargin);
%function [Infarr,fvec,Scarr] = sload({nfiles,strdate});
%returns fvec, frequency vector, and Scarr, complex S11 array read by readsdmpm.m
...
Scarr = [];
for n=1:nfiles
   [Info,fvec,S] = readsdmpm(strdate,n);
   Scarr = [Scarr S];
