...
class Measure(tk.Frame):
    ...
    def takecal(self):
        ...
        self.status.insert(END, f'1 of {nave+1} scans complete')
        self.status.yview(END)
        for n in range(nave):  #averaging
            ...
            self.status.insert(END, f'{n+2} of {nave+1} scans complete')
            self.status.yview(END)
...
class Viewer2(tk.Frame):
    ...
    def save_as(self):
        filename = tk.filedialog.asksaveasfilename(initialdir='/home/fmrdata/Data/FMRanalysis', defaultextension="*.txt")
...
#
    ...
    def chi(self):
        ...
        fmt='%.5e'
        dataout=np.column_stack((self.freq, chi, chii, texti['S11m'].to_numpy(dtype=complex).real, texti['S11m'].to_numpy(dtype=complex).imag))
    ...
    def fit_chi(self):
        ...
        freqlin=freq[np.where(freq>end)]
        freqpre=freq[np.where((freq<start) & (freq>0.5e10))]
        chiilin=chii[np.where(freq>end)]
        chiipre=chii[np.where((freq<start) & (freq>0.5e10))]
        chireal=self.chii.real[np.where(start<freq)]
        chiilin=np.concatenate((chiipre, chiilin),axis=None)
        freqlin=np.concatenate((freqpre, freqlin),axis=None)
        #maxid=max(chireal)
        #freqmax=self.freq[np.where(self.chii.real==maxid)]
        #minid=min(chireal)
        #freqmin=self.freq[np.where(self.chii.real==minid)]
        #shiftid=(max(chireal)-min(chireal))*0.5
        shiftid=np.mean(self.chii.real[np.where((self.freq>(self.mode-self.mode*0.1)) & (self.freq<(self.mode+self.mode*0.1)))])
        paralin=Parameters()
        paralin.add('a',value=0)
        paralin.add('b', value=-1E-6)
        paralin.add('c',value =-0.001)
        paralin.add('d',value=0)
        def linfunc(paralin,x,y):
            ...
            return resids
        minne = Minimizer(linfunc, paralin,fcn_args=(freqlin,chiilin))#-shiftid))
        resultlin = minne.minimize()
        report_fit(resultlin)
        if self.varBackgr.get()==1:
            a=resultlin.params['a'].value
            b=resultlin.params['b'].value
            c=resultlin.params['c'].value
            d=resultlin.params['d'].value
            tverhluti=self.chii.imag-a*self.freq**3-b*self.freq**2-c*self.freq-d
            raunhluti=self.chii.real
    ...
    def file_open(self):
        t=tk.filedialog.askopenfilenames(initialdir='/home/fmrdata/Data',filetypes=(('dat files', '*.dat'),('all files','*.*')))
    ...
    def fitchiframe(self):
        if self.fitchivar.get()==1:
           ...
           # self.entryKs.grid(row=5,column=1)
            ...
            tk.Label(self.custframe,text='M\u0073 [emu/cm^3]').grid(row=5,column=0)
            ...
            tk.Button(self.custframe, text='Export report', command=self.exportPdf).grid(row=8,column=0)

            self.varLeastsq, self.varShgo=tk.IntVar(self), tk.IntVar(self)
            self.Leastsq=tk.Checkbutton(self.custframe, text="Leastsq", variable=self.varLeastsq,command=lambda: self.set_checkfit(2)).grid(row=8, column=1)
            self.Shgo=tk.Checkbutton(self.custframe, text="Shgo", variable=self.varShgo,command=lambda: self.set_checkfit(3)).grid(row=8,column=2)
            self.varLeastsq.set(1)
            self.varBackgr=tk.IntVar(self)
            self.Backgr=tk.Checkbutton(self.custframe, text="Backgr", variable=self.varBackgr).grid(row=8,column=3)
            self.varBackgr.set(1)
            tk.Button(self.custframe, text='Resize', command=self.fit_chi).grid(row=9,column=0)
            self.entryLower=tk.Entry(self.custframe,width=5)
            self.entryLower.insert(0,'0')
            self.entryLower.grid(row=9,column=1)
            self.entrySuperior=tk.Entry(self.custframe,width=5)
            self.entrySuperior.insert(0,'0')
            self.entrySuperior.grid(row=9, column=2)
    ...
    def plot(self):
        ...
        for item in plotlist:
            ...
            if selected == 'angle':
                ...
                self.ax.clear()
                self.ax.set_xlabel('$\Theta [rad]$')
                self.ax.set_ylabel('($\omega/\gamma$)^2 [Oe$^2$]')
                self.ax.plot(angle, (resonance/gamma)**2,'*')
                ...
                def fcn2min(params, x, y):
                    ...
                    Y=Gamma**2*(x+4*np.pi*M+2*Ku/M)*(x+2*Ku/M)
                    resids = (Y-y)**2
